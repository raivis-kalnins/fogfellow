 ...
 * @package    WordPress
 * @subpackage Glass
 * @link       http://wearesmile.com/
 * @author     Waren Reeves <warren@waresmile.com>
 * @copyright  We are SMILE Ltd.
 */
 ...
 *
 * @package Glass
 * @author  Freekbron (https://gist.github.com/Freekbron/b8cb016bfcf25967c60a_)
 */
class Bulma_Menu_Walker extends Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$output .= $indent . '<input aria-label="Toggle Sub menu items" tabindex="-1" type="checkbox" id="sub-menu-' . esc_attr( $this->end_count ) . '" class="toggle-sub-mobile hidden hidden--fixed" aria-hidden="true" /><label class="navbar-toggle--submenu" aria-hidden="true" aria-label="Toggle Sub menu items" for="sub-menu-' . esc_attr( $this->end_count ) . '"><span class="hidden">Toggle Sub menu items</span><span class="is-icon fas fa-chevron-down"></span></label><div class="navbar-dropdown is-boxed';
		...
		if ( ! $args['has_children'] ) {// if doesn't have children
			if ( ! in_array( 'navbar-divider', $classes ) ) {// if doesnt contains a navbar divider
				...
				if ( ! empty( $fa ) ) {// if item title is not empty
					$item_output .= '<a' . $class_names . $attributes . '><span class="is-icon ' . $fa . '"></span>';
					if ( is_bool( $the_title ) ) {
						$show_title = false;// don't display text
					}
				} else {
					$item_output .= '<a role="menuitem"' . $class_names . $attributes . '>';// item empty use defaults
				}
			...
			$item_output .= $indent . '<div class="navbar-item has-dropdown is-hoverable" data-target="dropdown">' . "\n";
			if ( ! empty( $fa ) ) {// if fa  is not empty
				$item_output .= '<a role="menuitem"' . $class_names . $attributes . '><span class="is-icon ' . $fa . '"></span>';
				if ( is_bool( $the_title ) ) {
					$show_title = false;// don't display text
				}
			} else {
				$item_output .= '<a role="menuitem"' . $class_names . $attributes . $id . '>';// output standard
			}
	...
	private function get_item_classes( $item, $args ) {
		$classes     = empty( $item->classes ) ? array() : (array) $item->classes;
		$classes[]   = 'is-uppercase faded-to-light menu-item-' . $item->ID;
		$classes[]   = 'navbar-item';
		$class_names = join( ' ', apply_filters( 'nav_menu_css_class', array_filter( $classes ), $item, $args ) );
		if ( $args['has_children'] ) {
			$class_names .= ' dropdown navbar-link is-arrowless faded-to-light';
		}
		if ( in_array( 'current-menu-item', $classes, true ) ) {
			$class_names .= ' is-active faded-to-light';
		}
